...
export const GuardListScreen = () => {
  ...
  const [appliedResidencialId, setAppliedClientId] = useState<string | number>(
    'ALL',
  );
  const [tempResidencialId, setTempClientId] = useState<string | number>('ALL');
  const [selectedRole, setSelectedRole] = useState<string | null>(null);

  // Reassignment States
  const [changingGuard, setChangingGuard] = useState<any>(null);
  const [showResidencialModal, setShowClientModal] = useState(false);
  ...
  const fetchGuards = useCallback(
      ...
      try {
        ...
        const params = {
          ...
          filters: {
            ...
            role: roleFilter,
            ...(appliedResidencialId !== 'ALL' && { residencialId: appliedResidencialId }),
          },
        };
    ...
    [debouncedSearch, user.role, appliedResidencialId, selectedRole, dispatch],
  );

  useEffect(() => {
    getClients().then(res => {
      if (res.success && res.data) {
        setResidenciales(res.data.map((c: any) => ({ label: c.name, value: c.id })));
      }
    });
  ...
  const handleApplyFilters = () => {
    setAppliedClientId(tempResidencialId);
  ...
  const activeFiltersCount = appliedResidencialId !== 'ALL' ? 1 : 0;
        ...
        <View style={styles.cardBody}>
          ...
          <View style={styles.infoItem}>
            ...
            <ITText
              ...
              numberOfLines={1}
            >
              {item.client ? item.residencial.name : 'Sin Residencial Asignada'}
            </ITText>
          </View>
        </View>
        ...
        {user.role !== UserRole.RESDN && (
          <View style={styles.cardFooter}>
            ...
            >
              ...
              <ITText style={styles.footerButtonText}>Residencial</ITText>
            </ITTouchableOpacity>
          </View>
        )}
      </ITCard>
        ...
        >
          ...
          <Dialog.Content>
            {updating ? (
              ...
              <FlatList
                data={residenciales.filter(r => r.value !== 'ALL')}
                keyExtractor={item => item.value.toString()}
                renderItem={({ item }) => {
                    const isSelected = changingGuard?.residencial?.id === item.value;
                  return (
                    <List.Item
                      title={item.label}
                      onPress={() =>
                        handleUpdate(changingGuard.id, { residencialId: item.value })
                      }
                      left={props => (
                        <List.Icon
                          {...props}
        ...
        >
          ...
          >
            <View style={styles.filterGroup}>
              ...
              <SearchComponent
                label="Residencial"
                placeholder="Todas las residenciales"
                options={[
                  { label: 'Todas las residenciales', value: 'ALL' },
                  ...residenciales,
                ]}
                value={tempResidencialId}
