...
import { getClients } from '../../residenciales/service/residencial.service';
...
export const AssignmentModal = ({
  ...
  onDismiss,
  guardId,
  residencialId: initialResidencialId,
  onSuccess,
}: Props) => {
  ...
  const [loadingResidenciales, setLoadingClients] = useState(false);
  const [locationOptions, setLocationOptions] = useState<
    { label: string; value: string }[]
  >([]);
  const [residencialOptions, setResidencialOptions] = useState<
    { label: string; value: string }[]
  >([]);

  // State
  const [activeResidencialId, setActiveClientId] = useState<
    string | number | undefined
  >(initialResidencialId);
  const [selectedLocationId, setSelectedLocationId] = useState<string | null>(
    null,
  );
  const [selectedResidencialId, setSelectedClientId] = useState<string | null>(null);
  ...
  useEffect(() => {
    if (visible) {
      setActiveClientId(initialResidencialId);
      loadData(initialResidencialId);
      // Reset Form
  ...
  const loadData = async (cId?: string | number) => {
    ...
    try {
      ...
      const availableLocations = allLocations.filter(
        l => l.residencialId === cId && !busyLocationIds.includes(l.id),
      );
  ...
  const handleReassignClient = async () => {
    if (!selectedResidencialId) return;

    setReassigning(true);
    dispatch(showLoader(true));
    try {
      const res = await updateUser(guardId, { residencialId: selectedResidencialId });
      if (res.success) {
        Toast.show({ type: 'success', text1: 'Residencial Asignada' });
        setActiveClientId(selectedResidencialId);
        loadData(selectedResidencialId);
      }
    } catch (error) {
      console.error(error);
      Toast.show({ type: 'error', text1: 'Error al asignar residencial' });
    } finally {
    ...
    <Modal visible={visible} onRequestClose={onDismiss} animationType="slide">
      ...
      >
        ...
        <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
          {!activeResidencialId ? (
            <View style={styles.noResidencialContainer}>
              ...
              >
                Este guardia no tiene una residencial asignada. Seleccione una para
                ver sus ubicaciones disponibles.
              </ITText>

              <View style={{ width: '100%' }}>
                  <SearchComponent
                    label="Seleccionar Residencial"
                    placeholder="Buscar residencial..."
                  options={residencialOptions}
                  value={selectedResidencialId || ''}
                  onSelect={val => setSelectedClientId(val)}
                  loading={loadingResidenciales}
                />
                  <ITButton
                    label="ASIGNAR RESIDENCIAL"
                  onPress={handleReassignClient}
                  disabled={!selectedResidencialId || reassigning}
                  loading={reassigning}
                  style={{ marginTop: 24 }}
                />
                <ITButton
            ...
            <>
              <View style={styles.section}>
                <View style={styles.sectionHeader}>
                  ...
                  <ITBadge
                    label="Filtrado por Residencial"
                    variant="surface"
                    size="small"
                  />
                </View>
                {loadingLocations ? (
                  ...
                  <>
                    ...
                    {locationOptions.length === 0 && !loadingLocations && (
                      ...
                      >
                        No hay ubicaciones disponibles para esta residencial.
                      </ITText>
                    )}
                  </>
                )}
