...
export const KardexScreen = () => {
  ...
  const [locationsCatalog, setLocationsCatalog] = useState<
    { label: string; value: number }[]
  >([]);
  const [residencialesCatalog, setResidencialesCatalog] = useState<
    { label: string; value: number }[]
  >([]);
  ...
  const activeFiltersCount = [
    ...
    appliedFilters.residencialId ? 1 : 0,
  ].reduce((a, b) => a + b, 0);
      ...
      <View style={styles.header}>
        ...
        >
          <TouchableOpacity
            style={[
              styles.clientBadge,
              !appliedFilters.residencialId && styles.clientBadgeActive,
            ]}
            onPress={() => {
              setAppliedFilters(prev => ({ ...prev, residencialId: undefined }));
              setPage(1);
            }}
          >
            <ITText
              ...
              weight="bold"
              color={
                !appliedFilters.residencialId ? '#FFFFFF' : theme.colors.outline
              }
            >
              Todos
            </ITText>
          </TouchableOpacity>
          {residencialesCatalog.map(r => {
            const isActive = appliedFilters.residencialId === String(r.value);
            return (
              <TouchableOpacity
                key={r.value}
                style={[
          ...
          >
            ...
            <View style={styles.filterGroup}>
              ...
              <SearchComponent
                ...
                options={residencialesCatalog}
                value={tempFilters.residencialId}
                onSelect={val =>
                  setTempFilters({
                    ...tempFilters,
                    residencialId: val ? String(val) : undefined,
                  })
                }
              />
            </View>
