...
export const UserFormScreen = () => {
  ...
  const preselectedResidencialId = route.params?.residencialId;
  const isEdit = !!user;
  ...
  useEffect(() => {
    ...
    getCatalog('client').then(res => {
      if (res.success && res.data) setResidenciales(res.data);
    });
  }, [allowedRoles]);
  ...
  const handleSubmit = async (values: any) => {
    ...
    // Sanitize residencialId: If not operational, it should be null
    const isOperational =
            ...
            initialValues={{
              ...
              role: user
                ? typeof user.role === 'object'
                  ...
                  : user.role
                : '',
              residencialId: user?.residencial?.id || preselectedResidencialId || '',
