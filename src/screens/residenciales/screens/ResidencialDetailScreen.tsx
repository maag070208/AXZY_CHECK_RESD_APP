...
import { ResidencialStackParamList } from '../stack/ResidencialStack';
import { getClientById } from '../service/residencial.service';
...
type ClientDetailRouteProp = RouteProp<ResidencialStackParamList, 'CLIENT_DETAIL'>;
...
export const ResidencialDetailScreen = () => {
  const route = useRoute<ClientDetailRouteProp>();
  const navigation = useNavigation<any>();
  const { id } = route.params;
  const dispatch = useDispatch();

  const [residencial, setResidencial] = useState<IResidencial | null>(null);
                  ...
                  <View style={styles.profileHeader}>
                    <View style={styles.avatarContainer}>
                      <ITText style={styles.avatarText}>
                        {residencial.name?.charAt(0).toUpperCase()}
                      </ITText>
                    </View>
                    ...
                    <View style={styles.profileInfo}>
                      ...
                      <View style={styles.statusContainer}>
                        <ITBadge
                          label={residencial.active ? 'Activo' : 'Inactivo'}
                          variant={residencial.active ? 'success' : 'error'}
                          size="small"
                          dot={residencial.active}
                        />
                      </View>
                    </View>
              ...
              <View style={styles.menuSection}>
                ...
                <View style={styles.menuList}>
                  {renderMenuItem(
                    ...
                    'CLIENT_LOCATIONS',
                    `${residencial._count?.locations || 0} ubicaciones registradas`,
                  )}
                  {renderMenuItem(
                    'Zonas de Recorrido',
                    'layers-outline',
                    'CLIENT_ZONES',
                    `${residencial._count?.zones || 0} zonas configuradas`,
                  )}
                  {renderMenuItem(
                    'Guardias Asignados',
                    'shield-account-outline',
                    'CLIENT_GUARDS',
                    `${residencial._count?.users || 0} guardias asignados`,
                  )}
                </View>
              </View>
            </>
