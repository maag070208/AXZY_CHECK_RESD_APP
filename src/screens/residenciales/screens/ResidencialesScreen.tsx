...
export const ResidencialesScreen = ({ navigation }: any) => {
  ...
  const fetchData = async (pageNum: number, isRefreshing = false) => {
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        if (pageNum === 1) {
          setResidenciales(newRows);
          setHasMore(newRows.length < totalRows);
        } else {
          setResidenciales(prev => [...prev, ...newRows]);
          setHasMore(residenciales.length + newRows.length < totalRows);
        }
  ...
  const handleEdit = (item: IResidencial) => {
    navigation.navigate('CREATE_CLIENT', { residencial: item });
  };

  const handleDelete = (item: IResidencial) => {
    Alert.alert(
      ...
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          style: 'destructive',
          onPress: async () => {
            const res = await deleteResidencial(item.id);
            if (res.success) {
              dispatch(
                showToast({ message: 'Residencial eliminada', type: 'success' }),
              );
              fetchData(1);
            }
          },
  ...
  const renderItem = ({ item }: { item: IResidencial }) => (
    <Card
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        <View style={styles.headerTop}>
          <View>
            <Text style={styles.headerTitle}>Residenciales</Text>
            <Text style={styles.headerSubtitle}>
              {total} residenciales registradas
            </Text>
          </View>
        </View>
        <Searchbar
      ...
      {loading && !refreshing ? (
        <View style={styles.loadingContainer}>
          ...
          <Text style={styles.loadingText}>Cargando residenciales...</Text>
        </View>
      ) : (
        <FlatList
          data={residenciales}
          ...
          ListEmptyComponent={
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptyTitle}>Sin residenciales</Text>
              <Text style={styles.emptyText}>
                No se encontraron residenciales registradas.
              </Text>
            </View>
          }
        />
