...
export const GuardDashboard = () => {
  ...
  const loadData = async (isSilent = false) => {
    ...
    try {
      ...
      setResidenciales(filteredRecurring);
  ...
  const handleToggleRound = async () => {
    if (!isRoundActive) {
      ...
      residenciales.length === 1
        ? onStartRoundConfirmed(residenciales[0].id)
        ...
        <>
          ...
          <View style={styles.contentSheet}>
            ...
            <FlatList
              data={
                isMyRound
                  ? residenciales.filter(
                      r => r.id === activeRound.recurringConfigurationId,
                    )
                  : residenciales
              }
