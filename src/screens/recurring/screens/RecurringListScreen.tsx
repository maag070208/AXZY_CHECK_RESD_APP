...
export const RecurringListScreen = ({ navigation }: any) => {
  ...
  const [appliedResidencialId, setAppliedClientId] = useState<number | string>('');
  const [tempResidencialId, setTempClientId] = useState<number | string>('');
  ...
  const loadResidenciales = async () => {
    try {
      ...
      console.error('Error loading residenciales for filter:', error);
    }
  };
  ...
  useEffect(() => {
    fetchData(1);
  }, [debouncedSearch, appliedResidencialId]);
  ...
  const fetchData = async (pageNum: number, isRefreshing = false) => {
    ...
    const params = {
      ...
      filters: {
        ...
        residencialId: appliedResidencialId || undefined,
      },
    };
  ...
  const handleOpenFilters = () => {
    setTempClientId(appliedResidencialId);
    setShowFilters(true);
  };
  ...
  const residencialOptions = residenciales.map(r => ({
    label: r.name,
    value: String(r.id),
  }));

  const activeFiltersCount = appliedResidencialId !== '' ? 1 : 0;

  const renderItem = ({ item }: { item: any }) => {
    const residencialName =
      item.residencial?.name ||
      item.recurringLocations?.[0]?.location?.residencial?.name ||
      'N/A';
    const firstLocation =
        ...
        <View style={styles.cardLayout}>
          ...
          <View style={styles.infoSection}>
            ...
            <ITText variant="labelSmall" weight="bold" color="#94A3B8">
              RESIDENCIAL: {residencialName.toUpperCase()}
            </ITText>
        ...
        filterBadges={
          appliedResidencialId ? (
            ...
            >
              <ITBadge
                label={`Residencial: ${
                  residenciales.find(r => String(r.id) === String(appliedResidencialId))
                    ?.name || '...'
                }`}
                variant="primary"
      ...
      >
        <View style={styles.filterGroup}>
          ...
          <SearchComponent
            ...
            value={String(tempResidencialId)}
