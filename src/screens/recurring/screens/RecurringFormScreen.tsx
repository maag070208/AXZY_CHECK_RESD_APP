...
export const RecurringFormScreen = ({ navigation, route }: any) => {
  ...
  const [selectedResidencialId, setSelectedClientId] = useState<string>('');
  ...
  const loadInitialData = async () => {
    ...
    try {
      ...
      if (clientRes.success) setResidenciales(clientRes.data || []);
      ...
      if (isEditing) {
        ...
        if (fullRes && fullRes.success && fullRes.data) {
          ...
          if (data.residencialId) {
            setSelectedClientId(data.residencialId);
          } else if (data.recurringLocations?.[0]?.location?.residencialId) {
            setSelectedClientId(data.recurringLocations[0].location.residencialId);
          }
        }
      } else {
  ...
  useEffect(() => {
    if (selectedResidencialId) {
      fetchZones(String(selectedResidencialId));
    } else {
      setZones([]);
      setSelectedZoneId('');
    }
  ...
  const fetchZones = async (residencialId: string) => {
    try {
      const res = await getPaginatedZones({ filters: { residencialId } });
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        title,
        residencialId: selectedResidencialId || undefined,
  ...
  const validateCurrentStep = () => {
    if (currentStep === 0) {
      if (!title.trim() || !selectedResidencialId) {
        dispatch(
          showToast({
            message: 'Completa el título y selecciona una residencial',
            type: 'warning',
          }),
        );
        return;
  ...
  const availableLocations = locations.filter(l => {
    ...
    if (selectedResidencialId && String(l.residencialId) !== String(selectedResidencialId))
      return false;
          ...
          <Card.Content>
            ...
            {currentStep === 0 && (
              <View>
                ...
                <View style={styles.inputGroup}>
                  <SearchComponent
                    label="Residencial Operativo"
                    placeholder="Selecciona residencial..."
                    options={residenciales.map(r => ({ label: r.name, value: r.id }))}
                  ...
                  <View style={styles.guardsGrid}>
                    {guards
                      .filter(
                        g =>
                          !selectedResidencialId ||
                          String(g.residencialId) === String(selectedResidencialId),
                      )
                      .map(guard => (
                        <ITTouchableOpacity
                          key={guard.id}
                ...
                <View style={styles.summaryContent}>
                  <View style={styles.summaryRow}>
                    <View style={styles.summaryItem}>
                      <Text style={styles.summaryLabel}>RESIDENCIAL</Text>
                      <Text style={styles.summaryValue}>
                        {residenciales.find(
                          r => String(r.id) === String(selectedResidencialId),
                        )?.name || 'N/A'}
                      </Text>
                    </View>
                    <View style={styles.summaryItem}>
