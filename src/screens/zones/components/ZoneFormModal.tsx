...
const ZoneSchema = Yup.object().shape({
  name: Yup.string().required('El nombre de la zona es requerido'),
    residencialId: Yup.string().required('Debes seleccionar una residencial'),
});
...
export const ZoneFormModal = ({
  ...
  const [fetchingResidenciales, setFetchingResidenciales] = useState(false);
  ...
  const loadResidenciales = async () => {
    ...
    try {
      ...
      console.error('Error loading residenciales for zone modal:', error);
    } finally {
      setFetchingResidenciales(false);
    }
  };
        ...
        >
          <Formik
            initialValues={{
              ...
              residencialId: initialData?.residencialId || '',
            }}
            enableReinitialize
                  ...
                  <View style={styles.formContent}>
                    <View style={styles.section}>
                      ...
                      <View style={styles.inputGroup}>
                        <SearchComponent
                          label="RESIDENCIAL *"
                          placeholder="Selecciona una residencial"
                          options={residenciales.map(r => ({
                            label: r.name,
                            value: r.id,
                          }))}
                          value={values.residencialId}
                          onSelect={val => setFieldValue('residencialId', val)}
                          error={touched.residencialId && errors.residencialId}
                        />
                      </View>
                    </View>
